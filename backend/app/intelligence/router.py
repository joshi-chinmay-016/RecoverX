"""API router for Revenue Intelligence."""
...
async def get_intelligence_overview(db: Session = Depends(get_db)):
    """Get merchant-level aggregate intelligence overview."""
...
async def get_opportunity(result_id: str, db: Session = Depends(get_db)):
    """Get detailed intelligence result for a specific opportunity."""
    service = IntelligenceService(db)
    result = service.get_intelligence_result(result_id)
...
async def analyze_payment(
    ...
    logger.info(f"payment_analysis_completed payment_id={payment_id}")
...
async def batch_analyze(
    ...
    """
    Run batch intelligence analysis.
    ...
    logger.info(f"batch_analysis_completed count={analyzed_count} errors={len(errors)}")
